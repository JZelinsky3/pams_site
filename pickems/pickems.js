// ── pickems.js — PA Milk Society Pick'ems ──
...
const COLLECTION = "pams_pickems_votes";   // separate from jzff
...
async function onSubmitPicks() {
  ...
  if (snap.exists()) { alert("Picks already submitted for this week."); return; }
...
async function loadExistingSubmission(w) {
  ...
  const snap = await getDoc(doc(db, COLLECTION, `${w.id}_${state.user.name}`));
  if (!snap.exists()) return;
  const data = snap.data();

  for (const [mid, tid] of Object.entries(data.picks || {})) {
    document.querySelectorAll(`.week[data-week="${w.id}"] .match[data-mid="${mid}"] .vote-btn`)
      .forEach(b => { b.dataset.picked = b.dataset.team === tid ? "true" : ""; b.disabled = true; });
  }
  if (data.hl?.highest) { const s = byId(`hl-high-${w.id}`); if (s) { s.value = data.hl.highest; s.disabled = true; } }
  if (data.hl?.lowest)  { const s = byId(`hl-low-${w.id}`);  if (s) { s.value = data.hl.lowest;  s.disabled = true; } }
...
function liveTally(w, revealAt) {
  const qRef = query(collection(db, COLLECTION), where("weekId", "==", w.id));
...
function initRecords() {
  ...
  if (!weeksWithWinners.length) renderRecords([]);
}
